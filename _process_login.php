...
function jfb_process_login()
{
    ...
    //Start logging
    global $jfb_log, $jfb_version, $opt_jfb_app_id, $jfb_homepage, $jfb_apiver;
    jfb_debug_checkpoint('start');
    $browser = jfb_get_browser();
    $jfb_log = "Starting login process (IP: " . $_SERVER['REMOTE_ADDR'] . ", User: " . $_POST['fbuid'] . ", App: " . get_option($opt_jfb_app_id) . ", Plugin: $jfb_version, WP: " . $GLOBALS['wp_version'] . ", API: " . $jfb_apiver . ", Browser: " . $browser['shortname'] . " " . $browser['version'] . " for " . $browser['platform'] . ")\n";
    ...
    if (!get_option($opt_jfb_disablenonce)) {
        if (wp_verify_nonce($_REQUEST[$jfb_nonce_name], $jfb_nonce_name) != 1) {
            ...
            if ($currUser->ID) {
                $msg = sprintf(__("User '%s' has already logged in via another browser session.", 'wp-fb-autoconnect'), $currUser->user_login) . "\n";
                $jfb_log .= $msg;
                j_mail("FB Double-Login: " . $currUser->user_login . " -> " . get_bloginfo('name'));
                die($msg . "<br /><br /><a href=\"" . $_POST['redirectTo'] . "\">" . __("Continue", 'wp-fb-autoconnect') . "</a>");
            }

            j_die("Nonce check failed, login aborted.\n<br/><br/>\n" . __("This error is usually due to your browser's privacy settings or a server-side caching plugin.  If you get this error on multiple browsers, please contact the site administrator.", 'wp-fb-autoconnect'));
        }
    ...
    if (!isset($_POST['redirectTo']) || !$_POST['redirectTo']) {
        ...
        j_die("Error: Missing redirect URL", !get_option($opt_jfb_email_logs_missingpost));
    }
    $redirectTo = esc_html($_POST['redirectTo']);
    ...
    $fbuser = jfb_api_get("https://graph.facebook.com/$jfb_apiver/me?access_token=$access_token&fields=id,name,first_name,last_name,email,link");
    ...
    $pic = jfb_api_get("https://graph.facebook.com/$jfb_apiver/me/picture?" . apply_filters("wpfb_avatar_size", "type=square", "thumb") . "&redirect=false&access_token=$access_token");
    $fbuser['pic_square'] = $pic['data']['url'];
    $pic = jfb_api_get("https://graph.facebook.com/$jfb_apiver/me/picture?" . apply_filters("wpfb_avatar_size", "type=large", "full") . "&redirect=false&access_token=$access_token");
    ...
    if (isset($fbuser['email']) && strlen($fbuser['email']) != 0 && strpos($fbuser['email'], 'proxymail.facebook.com') === FALSE) {
        ...
        $jfb_log .= "FB: Email privilege denied or address inaccessible.\n";
    ...
    if ($user_login_id) {
        ...
        if ($updateEmail) {
            ...
            $res = wp_update_user($user_upd);
            if (is_wp_error($res))
                $jfb_log .= "WP: Failed to update email, probably because another user already exists with the same address.\n";
        }
    ...
    if (!$user_login_id) {
        ...
        //Facebook is now returning such absurdly long "scoped" profile URLs that wp_insert_user() was truncating them,
        //causing the insertion to fail. I tried just forming my own from the app_scoped_user_id, but those turned out
        //to (sometimes) be invalid.  So it looks like I can *only* store urls in my own usermeta. wp_users.user_url's max
        //length is 100 & FB urls are regularly 200chars long (ugh). I store it in $jfb_url_meta_name, below.
        //$user_data['user_url'] = $fbuser["profile_url"];
        //$user_data['user_url'] = "https://www.facebook.com/app_scoped_user_id/$fb_uid";
        ...
        //NOTE: wp_insert_user is *supposed* to return wp_error on failure, but some failures just return an ID of 0...
        $jfb_log .= "WP: Inserting new user into database...\n";
        $user_login_id   = wp_insert_user($user_data);
        if (is_wp_error($user_login_id) || $user_login_id == 0) {
            $errMsg = "Error: wp_insert_user failed!<br /><br />";
            $errMsg .= __("If you get this error while running Wordpress MultiSite, it means you'll need to purchase the premium addon to enable full MultiSite support.  Please see here for more information:", 'wp-fb-autoconnect');
            $errMsg .= " <a href=\"$jfb_homepage#premium\"><b>Premium Addon</b></a><br/>";
            $errMsg .= __("If you're NOT using MultiSite, please report this bug to the plugin author on the support page:", 'wp-fb-autoconnect');
        ...
        $jfb_log .= "WP: Inserted user's ID is $user_login_id\n";

        //Success! Notify the site admin.
        //(Implementation comes from pre-4.3's version of wp_new_user_notification)
        //See: https://core.trac.wordpress.org/ticket/33209#comment:12
        $user_login_name = $user_data['user_login'];
    ...
    if (isset($fbuser['link'])) {
        update_user_meta($user_login_id, $jfb_url_meta_name, $fbuser['link']);
        $jfb_log .= "WP: Updated usermeta ($jfb_url_meta_name -> " . $fbuser['link'] . ")\n";
    } else
        $jfb_log .= "FB: User's profile link could not be obtained. Skipping update of usermeta " . $jfb_url_meta_name . "\n";
