            ...
            <p>
                ...
                <?php _e("If you'd like to revert to a previous version of the free plugin until you're ready to update the addon, previous versions can be downloaded from:", 'wp-fb-autoconnect') ?> <a target="prev" href='http://wordpress.org/extend/plugins/wp-fb-autoconnect/developers/'><b>here</b></a>
                    ...
                    if (!function_exists('json_decode')) {
                        ?><div class="error">
            <p><strong><?php _e("Warning", 'wp-fb-autoconnect') ?>:</strong> <?php _e("WP-FB-AutoConnect requires the JSON PHP extension to work.  Please install / enable it before attempting to use this plugin.", 'wp-fb-autoconnect') ?></p>
        </div><?php
                    }
 ...
 */
    function jfb_admin_page()
    {
        ...
        global $opt_jfb_app_id, $opt_jfb_api_key, $opt_jfb_api_sec, $opt_jfb_email_to, $opt_jfb_email_logs, $opt_jfb_email_logs_missingpost, $jfb_homepage;
        global $opt_jfb_ask_perms, $opt_jfb_mod_done;
        global $opt_jfb_bp_avatars, $opt_jfb_wp_avatars, $opt_jfb_valid, $opt_jfb_fulllogerr, $opt_jfb_disablenonce, $opt_jfb_show_credit;
        global $opt_jfb_username_style, $opt_jfb_reportstats, $opt_jfb_hidew3warning, $opt_jfb_hidewpenginewarning;
        global $opt_jfb_ask_link;

        $jfb_admin_nonce = 'jfb_admin_nonce';
        ?>
    <div class="wrap">
        ...
        <?php
            ...
            if (version_compare('5', PHP_VERSION, ">")) {
                ?><div class="error">
                <p><?php _e("Sorry, but as of v1.3.0, WP-FB AutoConnect requires PHP5.", 'wp-fb-autoconnect') ?></p>
            </div><?php
                            die();
                        }
                        if (function_exists('is_multisite') && is_multisite() && !jfb_premium()) {
                            ?><div class="error">
                <p><strong><?php _e("Warning", 'wp-fb-autoconnect') ?>:</strong> <?php _e("Wordpress MultiSite is only fully supported by the premium version of this plugin; please visit the documentation page for more details:", 'wp-fb-autoconnect') ?> <a target='no_multisite' href='<?php echo $jfb_homepage ?>#premium'><b>here</b></a></p>
            </div><?php
                        }
                        if (file_exists(realpath(dirname(__FILE__)) . "/WP-FB-AutoConnect-Premium.php")) {
                            ?><div class="error">
                <p><strong><?php _e("Notice", 'wp-fb-autoconnect') ?>:</strong>
                ...
                <p>
                    ...
                    Your site appears to be running in a foreign language, using an older version of the WP-FB-AutoConnect Premium addon. While the addon's functionality will work just fine, it may not reflect your site's selected language. This is due to a change since core plugin v4.2.0, to support localization on translate.wordpress.org.<br /><br />If you'd like to see the translated version of the premium addon, please download the latest addon from justin-klein.com/store. If you don't mind English, feel free to disregard this message.
                        ...
                        if (isset($_POST['fb_opts_updated'])) {
                            ...
                            // Verify the nonce before saving anything
                            if (!isset($_REQUEST[$jfb_admin_nonce])) {
                                die('Error: Missing nonce');
                            }
                            if(wp_verify_nonce($_REQUEST[$jfb_admin_nonce], $jfb_admin_nonce) != 1)
                            {
                        ...
                        <p><?php
                                            ...
                                            _e("This error message is being reported by the Facebook API - if you don't know what it means, please try searching for it in Google, and/or looking through the FAQs on the plugin support page.", "wp-fb-autoconnect")
                                            ?></p>
                    </div><?php
                                        } else {
                                            ?><div class="updated">
                        <p><strong><?php _e("Successfully connected with:", 'wp-fb-autoconnect') ?> <?php echo "'" . $result['name'] . "' (ID " . $result['id'] . ")"; ?></strong></p>
                    </div><?php
                                        }
                                    } else {
                                        $result = $response;
                                        ?><div class="error">
                    <p><?php _e("Failed to validate your App ID and Secret.", "wp-fb-autoconnect") ?><br />
                        Error Message: <i><?php echo (isset($result['error']['message']) ? $result['error']['message'] : "Unknown"); ?></i></p>
                </div><?php
                                }
                                ...
                                update_option($opt_jfb_api_key, sanitize_text_field($_POST[$opt_jfb_api_key]));
                                update_option($opt_jfb_api_sec, sanitize_text_field($_POST[$opt_jfb_api_sec]));
                            }
                            if (isset($_POST['main_opts_updated'])) {
                                ...
                                $shownTab = 1;
                                update_option($opt_jfb_ask_perms, isset($_POST[$opt_jfb_ask_perms]) ? 1 : 0);
                                update_option($opt_jfb_wp_avatars, isset($_POST[$opt_jfb_wp_avatars]) ? 1 : 0);
                                update_option($opt_jfb_show_credit, isset($_POST[$opt_jfb_show_credit]) ? 1 : 0);
                                update_option($opt_jfb_email_to, sanitize_email(isset($_POST[$opt_jfb_email_to]) ? $_POST[$opt_jfb_email_to] : ''));
                                update_option($opt_jfb_email_logs, isset($_POST[$opt_jfb_email_logs]) ? 1 : 0);
                                update_option($opt_jfb_email_logs_missingpost, isset($_POST[$opt_jfb_email_logs_missingpost]) ? 1 : 0);
                                update_option($opt_jfb_fulllogerr, isset($_POST[$opt_jfb_fulllogerr]) ? 1 : 0);
                                update_option($opt_jfb_disablenonce, isset($_POST[$opt_jfb_disablenonce]) ? 1 : 0);
                                update_option($opt_jfb_username_style, wp_kses_post(isset($_POST[$opt_jfb_username_style]) ? $_POST[$opt_jfb_username_style] : 0));
                                update_option($opt_jfb_reportstats, isset($_POST[$opt_jfb_reportstats]) ? 1 : 0);
                                update_option($opt_jfb_ask_link, isset($_POST[$opt_jfb_ask_link]) ? 1 : 0);
                                ?><div class="updated">
                <p><strong><?php _e("Options saved", 'wp-fb-autoconnect') ?></strong></p>
            </div><?php
                        }
                        ...
                        if (isset($_POST['remove_all_settings'])) {
                            ...
                            delete_option($opt_jfb_ask_link);
                            if (function_exists('jfb_delete_premium_opts')) jfb_delete_premium_opts();
                            ?><div class="updated">
                <p><strong><?php _e('All plugin settings have been cleared.', 'wp-fb-autoconnect'); ?></strong></p>
            </div><?php
                        }
                        ?>
        ...
        <?php
            ...
            $tabNames        = array(
                __("Facebook Setup", 'wp-fb-autoconnect'),
        ...
        <div class="jfb-admin_wrapper">
            <div class="<?php echo $allTabsClass ?>" id="<?php echo $tabIds[0] ?>" style="display:<?php echo ($shownTab == 0 ? "block" : "none") ?>">
                ...
                <?php _e("To allow your users to login with their Facebook accounts, you must first setup a Facebook Application for your website. Note that I'll do my best to keep these steps updated, but Facebook constantly moves things around on their site:", 'wp-fb-autoconnect') ?><br /><br />
                <ol>
                    <li><?php echo sprintf(__("Visit %s and click 'Create App.'", 'wp-fb-autoconnect'), "<a href='http://developers.facebook.com/apps' target='lnk'>developers.facebook.com/apps</a>") ?></li>
                    <li><?php _e("If it asks you to select a use case, choose 'Facebook Login.' If it asks you to select a type of app, choose 'Consumer.'", 'wp-fb-autoconnect') ?></li>
                    <li><?php _e("Type in a name (i.e. the name of your site) &amp; contact e-mail, and click the 'Create App' button.", 'wp-fb-autoconnect') ?></li>
                    <li><?php _e("Go to Settings-&gt;Basic (on the left), scroll down &amp; click 'Add Platform' (at the bottom), 'Website,' then fill in your 'Site URL.'  Save Changes. It may prompt you to fill out any additional required fields.", 'wp-fb-autoconnect') ?><br />
                    ...
                    <li><?php _e("Go to Products (on the left), then Configure-&gt;Settings next to 'Facebook Login.' On that page, toggle the 'Login with the JavaScript SDK' option to 'Yes', and click 'Save.' It may prompt you to fill out any additional required fields.", "wp-fb-autoconnect") ?></li>
                    <li><?php _e("On the left, go to 'Go live,' then click the 'Go Live' button on the bottom. It may prompt you to fill out any additional required fields.", 'wp-fb-autoconnect') ?></li>
                    <li><?php _e("Go to App Review (on the left)-&gt;Request Permissions or Features-&gt;click 'Edit' next to 'Authentication and account creation.' Then next to 'email', click 'Add.'", 'wp-fb-autoconnect') ?></li>
                    <li><?php echo sprintf(__("That's it for the Facebook app. If you're using a Wordpress theme that supports Widgets, just navigate to Appearance->Widgets (%s) in your Wordpress panel add the WP-FB AutoConnect widget to your site. If not, you'll need to install the Legacy Widget plugin (%s), then go to your Site Editor (%s). Add a Legacy Widget block wherever you like, and in that block, you can select the WP-FB AutoConnect Widget.", 'wp-fb-autoconnect'), "<a href='" . admin_url('widgets.php') . "' target='widgets'><b>here</b></a>", "<a href='https://wordpress.org/plugins/x3p0-legacy-widget/' target='_blank'><b>here</b></a>", "<a href='" . admin_url('site-editor.php') . "?canvas=edit' target='_blank'><b>here</b></a>"); ?></li>
                </ol>
                <br />
                ...
                <form name="formFacebook" method="post" action="">
                    ...
                    <?php wp_nonce_field($jfb_admin_nonce, $jfb_admin_nonce) ?>
                    <input type="hidden" name="fb_opts_updated" value="1" />
                    <div class="submit"><input type="submit" name="Submit" class="button button-primary" value="<?php _e("Connect", 'wp-fb-autoconnect') ?>" /></div>
                </form>
            </div> <!-- End Tab -->
            ...
            <div class="<?php echo $allTabsClass ?>" id="<?php echo $tabIds[1] ?>" style="display:<?php echo ($shownTab == 1 ? "block" : "none") ?>">
                ...
                <form name="formMainOptions" method="post" action="">
                    ...
                    <br /><b><?php _e("Profile URL", 'wp-fb-autoconnect') ?>:</b><br />
                    <input type="checkbox" name="<?php echo $opt_jfb_ask_link ?>" value="1" <?php echo get_option($opt_jfb_ask_link) ? 'checked="checked"' : '' ?> /> <?php _e("Request permission to get the connecting user's profile URL", 'wp-fb-autoconnect') ?>
                    <?php jfb_output_simple_lightbox("", __("For this option to work, you will need to submit your app for review by Facebook.  Go to the app's settings, then 'App Review' &amp; ask for the 'user_link' permission. Once they approve you, users who login will have their profile URL fetched &amp; listed on their WP profile page (assuming they don't explicitly deny the permission when they login).", 'wp-fb-autoconnect')) ?><br />
                    ...
                    <?php wp_nonce_field($jfb_admin_nonce, $jfb_admin_nonce) ?>
                    <div class="submit"><input type="submit" name="Submit" class="button button-primary" value="<?php _e("Save", 'wp-fb-autoconnect') ?>" /></div>
                </form>

                <?php if (!defined('JFB_PREMIUM')) : ?>
                    <hr />
                    <h3><?php _e('Donate', 'wp-fb-autoconnect') ?></h3>
                    <?php
                            echo sprintf(__("Countless hours have gone into developing, maintaining, & supporting this plugin.  Just keeping it running requires ongoing work due to Facebook's ever-changing API. If you find it useful, please consider supporting its continued development by purchasing the %s Premium addon %s, or making a donation of any amount.", 'wp-fb-autoconnect'), "<a href='$jfb_homepage#premium' target='buyprem'>", "</a>") ?><br /><br />
                    ...
                    <div style="float:left;text-align:center;width:200px;">
                        ...
                        <img width="150" height="150" src="<?php echo plugins_url(dirname(plugin_basename(__FILE__))) ?>/assets/qr.png" alt="QR" />
                    </div>
                    <div class="donateButton" style="float:left;text-align:center;width:200px;">
                        <b>PayPal:</b><br />
                        <form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top">
                            <input type="hidden" name="cmd" value="_s-xclick" />
                            <input type="hidden" name="hosted_button_id" value="A7BTQH6PBFMGN" />
            ...
            <div class="<?php echo $allTabsClass ?>" id="<?php echo $tabIds[3] ?>" style="display:<?php echo ($shownTab == 3 ? "block" : "none") ?>">
                ...
                <form name="formDebugOptions" method="post" action="">
                    ...
                    <?php wp_nonce_field($jfb_admin_nonce, $jfb_admin_nonce) ?>
                    <div class="submit"><input type="submit" name="Submit" class="button button-primary" value="<?php _e("Delete", 'wp-fb-autoconnect') ?>" /></div>
                </form>
            </div> <!-- End Tab -->
...
Facebook Graph:     <?php echo (is_wp_error($result) ? "ERROR" : $result['headers']['facebook-api-version']) . "\n"; ?>
Facebook Validated: <?php echo (get_option($opt_jfb_valid) ? "OK" : "NO") . "\n"; ?>
Allow Url fopen:    <?php echo ini_get("allow_url_fopen") . "\n"; ?>
Server:             <?php echo substr($_SERVER['SERVER_SOFTWARE'], 0, 65) . (strlen($_SERVER['SERVER_SOFTWARE']) > 65 ? "..." : ""); ?>  
Browser:            <?php $browser = jfb_get_browser();
...
function jfb_output_premium_panel_tease()
{
    ...
    global $opt_jfbp_email_rejected_msg, $opt_jfbp_avatar_full_dimensions_w, $opt_jfbp_avatar_full_dimensions_h, $opt_jfbp_email_notsetup, $opt_jfbp_email_notsetup_exception;
    ...
    <form name="formPremOptions" method="post" action="">
        ...
        <?php add_option($opt_jfbp_email_rejected_msg, __("Sorry, this site requires an email address to log you in.", 'wp-fb-autoconnect')); ?>
        <?php add_option($opt_jfbp_email_notsetup, __("Sorry, this site requires an e-mail address to log you in, but it appears that you do not have an e-mail setup on your Facebook account. Please login to Facebook &amp; register an e-mail, then try to login again.", 'wp-fb-autoconnect')); ?>
        <input <?php disableatt() ?> type="checkbox" class="noemailexception" name="<?php echo $opt_jfbp_email_notsetup_exception ?>" value="1" <?php echo get_option($opt_jfbp_email_notsetup_exception) ? 'checked="checked"' : '' ?> /> <?php _e("...Grant exception for Facebook accounts that don't have an e-mail (phone number only)", 'wp-fb-autoconnect') ?><br />
        <?php _e("Message to users who deny e-mail access:", 'wp-fb-autoconnect') ?> <input <?php disableatt() ?> type="text" size="55" name="<?php echo $opt_jfbp_email_rejected_msg; ?>" value="<?php echo get_option($opt_jfbp_email_rejected_msg); ?>" /><br />
        <div id="noemailexceptiontext" style="display:block;">
            <?php _e("Message to users whose FB accounts have no email (phone number only):", 'wp-fb-autoconnect') ?> <input <?php disableatt() ?> type="text" size="55" name="<?php echo $opt_jfbp_email_notsetup; ?>" value="<?php echo get_option($opt_jfbp_email_notsetup); ?>" /><br />
            <script type="text/javascript">
                jQuery(document).ready(function() {
                    if (jQuery('input.noemailexception').is(':checked')) jQuery('#noemailexceptiontext').hide();
                    jQuery('.noemailexception').change(function() {
                        if (this.checked)
                            jQuery('#noemailexceptiontext').hide();
                        else
                            jQuery('#noemailexceptiontext').show();
                    });
                });
            </script>
        </div><br />
...
-->
        ...
        &bull; <?php _e("Show/Hide any of the Widget's links, checkboxes, or textfields", 'wp-fb-autoconnect') ?> <?php jfb_output_simple_lightbox("", __("You can show or hide:", 'wp-fb-autoconnect') . "<ul style='list-style-type:disc;list-style-position:inside;'><li>" . __('The User/Pass fields (leaving Facebook as the only way to login via the widget)', 'wp-fb-autoconnect') . "</li><li>" . __("The 'Register' link (only applicable if registration is enabled on the site/network)", 'wp-fb-autoconnect') . "</li><li>" . __("The 'Remember' tickbox", 'wp-fb-autoconnect') . "</li><li>" . __("The 'Edit Profile' link", 'wp-fb-autoconnect') . "</li><li>" . __("The 'Forgot Password' link", 'wp-fb-autoconnect') . "</li></ul>") ?><br />
        ...
        <input <?php disableatt() ?> type="radio" name="<?php echo $opt_jfbp_restrict_reg; ?>" value="3" <?php echo (get_option($opt_jfbp_restrict_reg) == 3 ? "checked='checked'" : "") ?>><?php _e('Friendship - Only autoregister friends of Facebook user id:', 'wp-fb-autoconnect') ?> <input <?php disableatt() ?> type="text" size="15" name="<?php echo $opt_jfbp_restrict_reg_uid ?>" value="<?php echo get_option($opt_jfbp_restrict_reg_uid) ?>" /> <?php jfb_output_simple_lightbox("", __("For this feature to work:", 'wp-fb-autoconnect') . "<br/>*" . __("You'll need to submit your app for review by Facebook.  Please see FAQ#56 for more info:", 'wp-fb-autoconnect') . " <a href='$jfb_homepage#faq56' target='extendedfaq'>here</a><br/>*" . __("The user you enter must login via this Facebook app at least once after you've enabled the option, and must approve the 'Friends' permission when they do so.", 'wp-fb-autoconnect') . "<br/><br/>" . __("To find a Facebook user's numeric ID, you can use findmyfbid.com, or Google for the latest instructions. Unfortunately, Facebook frequently moves things around, meaning the ID's location on their site can differ from one day to the next.", "wp-fb-autoconnect")) ?><br />
        ...
        <input <?php disableatt() ?> type="radio" name="<?php echo $opt_jfbp_restrict_reg; ?>" value="5" <?php echo (get_option($opt_jfbp_restrict_reg) == 5 ? "checked='checked'" : "") ?>><?php _e('Page - Only autoregister users who have liked Facebook page id:', 'wp-fb-autoconnect') ?> <input <?php disableatt() ?> type="text" size="15" name="<?php echo $opt_jfbp_restrict_reg_pid ?>" value="<?php echo get_option($opt_jfbp_restrict_reg_pid); ?>" /> <?php jfb_output_simple_lightbox("", sprintf(__("To find a page's id, you can use findmyfbid.com - or if you're an admin, visit the page &amp; click 'About,' and it should be listed at the bottom.", 'wp-fb-autoconnect'), "https://www.facebook.com/pagename/photos/a.123.456.789/555") . "<br/><br/>" . __("In order to use this option, you'll need to submit your app for review by Facebook.  Please see FAQ#56 for more info:", 'wp-fb-autoconnect') . " <a href='$jfb_homepage#faq56' target='extendedfaq'>here</a>") ?><br />
        ...
        <small>&bull; <?php _e('Some limitations exist regarding which Facebook fields can be imported', 'wp-fb-autoconnect') ?></small> <?php jfb_output_simple_lightbox("", __("Because some Facebook fields are formatted differently, each one needs to be explicitly implemented.  I've included an initial selection of fields (i.e. Name, Gender, Birthday, etc), but if you need another field to be available, please request it on the support page and I'll do my best to add it to the next update.", 'wp-fb-autoconnect')) ?><br />
        <small>&bull; <?php _e("In order to import fields marked with an asterisk (i.e. Birthday*), you'll need to submit your app for review by Facebook.  Please see FAQ#56 for more info:", 'wp-fb-autoconnect');
                            echo " <a href='$jfb_homepage#faq56' target='extendedfaq'>here</a>"; ?></small><br /><br />
        ...
        <?php
            ...
            <input <?php disableatt() ?> type="radio" name="<?php echo $opt_jfbp_xprofile_map; ?>" value="2" <?php echo (get_option($opt_jfbp_xprofile_map) == 2 ? "checked='checked'" : "") ?>><?php _e('Map New And Returning Users', 'wp-fb-autoconnect') ?><br />
            <?php
                    ...
                    $allowed_mappings = array(
                        'textbox' => array(
                            'name' => "Name", 'first_name' => "First Name", 'middle_name' => "Middle Name", 'last_name' => "Last Name",
                            'gender' => "Gender*", 'link' => "Profile URL*",
                            "location" => "City*",
                    ...
                    while (bp_profile_groups()) {
                        ...
                        ?><div style="width:520px; padding:5px; margin:2px 0; background-color:#EEEDDA; border:1px solid #CCC;">
                    <?php
                        ...
                        <?php
                            ...
                            <option value="0">(<?php _e('No Mapping', 'wp-fb-autoconnect') ?>)</option>
                            <?php
        ...
        <div class="submit"><input <?php disableatt() ?> type="submit" name="Submit" class="button button-primary" value="<?php _e('Save Premium', 'wp-fb-autoconnect') ?>" /></div>
    </form>
<?php
}
